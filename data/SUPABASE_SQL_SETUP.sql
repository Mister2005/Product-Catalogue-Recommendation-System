-- ================================================
-- SHL Recommendations Database Schema for Supabase
-- ================================================
-- Run this script in Supabase SQL Editor:
-- 1. Go to https://zrvbcwqepqbabbvehxke.supabase.co
...
CREATE INDEX IF NOT EXISTS idx_assessments_job_level ON assessments(job_level);
CREATE INDEX IF NOT EXISTS idx_assessments_type ON assessments(type);
CREATE INDEX IF NOT EXISTS idx_assessments_remote_testing ON assessments(remote_testing);
CREATE INDEX IF NOT EXISTS idx_assessments_name_trgm ON assessments USING gin (name gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_assessments_description_trgm ON assessments USING gin (description gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_test_types_assessment ON test_types(assessment_id);
CREATE INDEX IF NOT EXISTS idx_industries_assessment ON industries(assessment_id);
CREATE INDEX IF NOT EXISTS idx_skills_assessment ON skills(assessment_id);
...
-- ================================================
-- VERIFICATION QUERY
-- Run this after the script completes to verify:
-- ================================================
-- SELECT 
--     schemaname,
--     tablename
...
BEGIN
    ...
    RAISE NOTICE '📊 Tables created: assessments, test_types, industries, languages, skills, recommendation_history, user_feedback';
    RAISE NOTICE '🔍 Indexes created for optimized queries';
    RAISE NOTICE '👁️ View created: assessment_details';
    RAISE NOTICE '';
    RAISE NOTICE '🎯 Next step: Run the data migration script from your application';
END $$;
